...
import { getAvailableTimeSlots, DAY_NAMES, getTimeDifferenceInHours, isValidForFinancialIntelligence } from './timeSlots';
 ...
 */
const scheduleCourse = (course, financialYearStart, quarterDates, availabilityGrid, courseHistory) => {
  const sessions = [];
  const isFinancialIntelligence = course.title.toLowerCase().includes('financial intelligence');
  
  // Calculate possible start dates based on cadence
  const possibleStartDates = calculatePossibleStartDates(
    course,
    financialYearStart,
    quarterDates
  );
  
  // Try to schedule each instance of the course
  possibleStartDates.forEach(startDate => {
...
/**
 * Calculate possible start dates for a course based on cadence
 * @param {Object} course - Course object
 * @param {Date} financialYearStart - Start of financial year (April 1)
 * @param {Array<Date>} quarterDates - Dates in the quarter
 * @returns {Array<Date>} Possible start dates
 */
const calculatePossibleStartDates = (course, financialYearStart, quarterDates) => {
  const possibleDates = [];
  ...
  // Generate dates based on cadence from the first weekday of financial year
  // Stop at financial year end to prevent courses from extending beyond the year
  while (currentDate <= financialYearEnd) {
    // Check if this date falls within the quarter
    if (currentDate >= quarterDates[0] && currentDate <= quarterDates[quarterDates.length - 1]) {
    ...
    // Move to next cadence period (cadence weeks after first session)
 ...
 */
const findSuitableSlot = (date, course, availabilityGrid, history, isFinancialIntelligence) => {
  const dateKey = format(date, 'yyyy-MM-dd');
  const dayData = availabilityGrid.get(dateKey);
  ...
  for (const slot of dayData.slots) {
    ...
    if (isFinancialIntelligence && !isValidForFinancialIntelligence(dayOfWeek, slot.start)) {
      continue;
    }
    ...
    if (lastInstance) {
      ...
      const timeDiff = getTimeDifferenceInHours(slot.start, lastInstance.startTime);
      if (timeDiff < 3) {
        continue;
      }
    }
...
const rollbackSessions = (sessions, availabilityGrid) => {
  sessions.forEach(session => {
    ...
    if (dayData) {
      const slotIndex = dayData.slots.findIndex(s => s.start === session.startTime);
      if (slotIndex !== -1) {
...
const calculateStatistics = (availabilityGrid, sessions) => {
  ...
  return {
    ...
    fullyBookedDates,
    utilizationPercentage: totalSlots > 0 ? ((totalSlots - availableSlots) / totalSlots * 100).toFixed(2) : 0,
  };
};
